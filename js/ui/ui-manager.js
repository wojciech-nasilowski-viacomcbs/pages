...
(function () {
  ...
  const uiManager = {
    ...
    handleSoundToggle(elements) {
      const muted =
        window.toggleMute && typeof window.toggleMute === 'function' ? window.toggleMute() : false;
    ...
    async showKnowledgeBaseArticle(slug, sessionManager) {
      ...
      try {
        ...
        if (dateEl)
          dateEl.textContent = `📅 ${new Date(article.created_at).toLocaleDateString('pl-PL')}`;
        ...
        // Treść HTML
        if (bodyEl)
          bodyEl.innerHTML = article.content || '<p class="text-gray-400">Brak treści.</p>';

        // Przycisk kopiowania linku (dla wszystkich)
        const copyLinkBtn = document.getElementById('kb-copy-link');
        if (copyLinkBtn) {
          copyLinkBtn.dataset.articleSlug = article.slug;
          copyLinkBtn.dataset.articleTitle = article.title;
        }
    ...
    async showKnowledgeBaseEditor(slug = null) {
      ...
      if (slug) {
        try {
          ...
          document.getElementById('kb-editor-input-tags').value = Array.isArray(article.tags)
            ? article.tags.join(', ')
            : '';
          document.getElementById('kb-editor-input-published').checked =
          ...
          window.alert('Nie udało się załadować artykułu do edycji');
    ...
    async deleteKnowledgeBaseArticle(id) {
      try {
        ...
        // Pokaż komunikat
        window.alert('Artykuł został usunięty');
      } catch (err) {
        console.error('Błąd usuwania artykułu:', err);
        window.alert('Nie udało się usunąć artykułu: ' + err.message);
      }
    },
