...
(function () {
  ...
  const contentTypeConfig = {
    quiz: {
      ...
      loadAndStartFn: id =>
        contentManager.loadAndStartQuiz(id, state, elements, sessionManager, uiManager, true),
    ...
    workout: {
      ...
      loadAndStartFn: id =>
        contentManager.loadAndStartWorkout(id, state, elements, uiManager, sessionManager),
      dataServiceSaveFn: (data, isPublic) => dataService.saveWorkout(data, isPublic),
      dataServiceUpdateStatusFn: (id, isPublic) =>
        dataService.updateWorkoutPublicStatus(id, isPublic)
    },
    listening: {
      tabName: 'listening',
      ...
      dataServiceUpdateStatusFn: (id, isPublic) =>
        dataService.updateListeningSetPublicStatus(id, isPublic)
    },
    article: {
      tabName: 'knowledge-base',
      featureFlagCheck: () => featureFlags.isKnowledgeBaseEnabled(),
      loadAndStartFn: slug => {
        // Przełącz na ekran bazy wiedzy i pokaż artykuł
        uiManager.showScreen('knowledge-base', state, elements, contentManager, sessionManager);
        if (uiManager.showKnowledgeBaseArticle) {
          return uiManager.showKnowledgeBaseArticle(slug, sessionManager);
        }
        throw new Error('Knowledge Base nie jest dostępny');
      }
      // Artykuły nie mają dataServiceSaveFn i dataServiceUpdateStatusFn w tym samym formacie
    }
  };
  ...
  async function handleDeepLink() {
    try {
      ...
      console.log(`🔗 Deep link detected: type=${type}, id=${id}, slug=${slug}`);

      // Sprawdź czy użytkownik jest zalogowany
      // UWAGA: Po wdrożeniu nowych polityk RLS, wszystkie treści (w tym artykuły) wymagają logowania
      if (!state.currentUser) {
        console.warn('⚠️ User not authenticated, cannot load shared content');

        // Pokaż przyjazny komunikat z informacją o typie treści
        let contentTypeName = 'treść';
        if (type === 'quiz') contentTypeName = 'quiz';
        else if (type === 'workout') contentTypeName = 'trening';
        else if (type === 'listening') contentTypeName = 'zestaw do nauki';
        else if (type === 'article') contentTypeName = 'artykuł';
      ...
      try {
        ...
        const identifier = type === 'article' ? slug : id;
        await config.loadAndStartFn(identifier);
  ...
  async function init() {
    ...
    try {
      ...
      if (
        lastTab &&
        ['workouts', 'knowledge-base', 'quizzes', 'listening', 'more'].includes(lastTab)
      ) {
    ...
    if (featureFlags.isQuizzesEnabled() && typeof window.initQuizEngine === 'function') {
      window.initQuizEngine(
        screen => uiManager.showScreen(screen, state, elements, contentManager, sessionManager),
        state
      );
    }
    if (featureFlags.isWorkoutsEnabled() && typeof window.initWorkoutEngine === 'function') {
      window.initWorkoutEngine(
        screen => uiManager.showScreen(screen, state, elements, contentManager, sessionManager),
        state
      );
    }
    if (featureFlags.isListeningEnabled() && typeof window.initListeningEngine === 'function') {
      window.initListeningEngine(
        screen => uiManager.showScreen(screen, state, elements, contentManager, sessionManager),
        state
      );
    }
    ...
    if (!handled) {
      ...
      const tabToShow =
        state.currentTab && enabledTabs.includes(state.currentTab)
          ? state.currentTab
    ...
    window.addEventListener('beforeunload', e => {
      ...
      const isActive =
        ...
        (window.listeningEngine &&
          window.listeningEngine.isPlaying &&
          window.listeningEngine.isPlaying()); // Odtwarza listening
  ...
  function attachEventListeners() {
    ...
    document.addEventListener('click', e => {
      if (
        !elements.userMenuDropdown.classList.contains('hidden') &&
    ...
    elements.quizHome.addEventListener('click', () => {
      if (window.resetMistakes && typeof window.resetMistakes === 'function') {
  ...
  async function checkAuthState() {
    try {
      state.currentUser = await window.authService.getCurrentUser();
