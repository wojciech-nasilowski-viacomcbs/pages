...
(function () {
  ...
  async function loadListeningSets() {
    ...
    try {
      ...
      if (elements.listeningListError) {
        ...
        elements.listeningListError.textContent =
          'Nie udało się załadować zestawów. Spróbuj ponownie.';
      }
    } finally {
  ...
  function renderListeningCards(sets) {
    if (!sets || sets.length === 0) {
      elements.listeningListCards.innerHTML =
        '<p class="text-gray-400 text-center py-8">Brak zestawów do wyświetlenia.</p>';
      return;
    }
  ...
  function switchLanguageOrder() {
    playerState.langOrder = playerState.langOrder === 'lang1-first' ? 'lang2-first' : 'lang1-first';
    updatePlayerUI();
  }
  ...
  function setupScreenTipListeners() {
    ...
    if (!isMobileDevice() && elements.screenTimeoutTip) {
