...
(function () {
  ...
  `;
    ...
    if (questionData.audioText && questionData.autoPlay !== false) {
      setTimeout(() => {
        speakText(
          questionData.audioText,
          questionData.audioLang || 'en-US',
          questionData.audioRate || 0.85
        );
      }, 500);
    }
  ...
  `;
    ...
    elements.answersContainer.querySelectorAll('.quiz-option').forEach(btn => {
      btn.addEventListener('click', () =>
        handleMultipleChoiceAnswer(questionData, parseInt(btn.dataset.index))
      );
    });
  }
  ...
  `;
    ...
    submitBtn.addEventListener('click', () =>
      handleFillInTheBlankAnswer(questionData, input.value)
    );
  ...
  function renderMatching(questionData) {
    ...
    const rightItemsWithIndex = questionData.pairs.map((p, idx) => ({
      text: p.match,
      originalIndex: idx
    }));
    // Losowa kolejność
    ...
    elements.answersContainer.querySelectorAll('.matching-left').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (existingMatch) {
          ...
          const rightBtn = elements.answersContainer.querySelector(
    ...
    elements.answersContainer.querySelectorAll('.matching-right').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (existingMatch) {
          ...
          const leftBtn = elements.answersContainer.querySelector(
        ...
        const leftBtn = elements.answersContainer.querySelector(
          `.matching-left[data-index="${leftIndex}"]`
        );
  ...
  function handleMatchingAnswer(questionData, userMatches) {
    ...
    if (isCorrect) {
      ...
      explanation +=
        '<div class="mt-3 text-sm"><strong>Prawidłowe dopasowania:</strong><ul class="mt-2 space-y-1">';
